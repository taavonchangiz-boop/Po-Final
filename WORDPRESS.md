...
- هر سایت یک جفت `publicId` (شناسه عمومی) + `secret` (کلید HMAC، در DB پُستیار رمزشده نگهداری می‌شود — AES-256-GCM؛ در پلاگین در `wp_options` ذخیره و در فرم هرگز بازنمایی کامل نمی‌شود).
- **وب‌هوک پلاگین→پُستیار:** `POST /api/v1/webhooks/wordpress/:publicId` با هدر `X-Postyar-Signature: sha256=HMAC(raw_body, site secret)`؛ tolerance زمان ۵ دقیقه (هدر timestamp)؛ بدنه ≤256KB؛ dedupe با event id در `wp_events`.
...
- sync فقط خواندنی است (پُستیار محصول وردپرس را تغییر نمی‌دهد)؛ فیلدها: عنوان، توضیح کوتاه، قیمت (BIGINT ریال)، تصویر شاخص URL، وضعیت انتشار.
...
- رویداد `product.published` (transition به `publish` با گارد once-only مثل `_whcm_auto_sent` ولی سمت SaaS: کلید idempotency `wp:{siteId}:{productId}`) ⇒ پُستیار پست «انتشار محصول» با قالب تعریف‌شده روی کانال‌های انتخابی می‌سازد ⇒ همان مسیر outbox→worker→delivery با ردیابی وضعیت per-channel.
- حالت دستی هم هست: از صفحه محصول وردپرس یا داشبارد پُستیار «انتشار محصول روی کانال‌ها».
...
- سمت پُستیار: `GET /wordpress/sites` (وضعیت، آخرین sync)، `GET /wordpress/sites/:id/products`، و رویدادهای اخیر در `wp_events`.
...
- حذف کامل (`uninstall.php`): حذف جداول/آپشن‌های خود افزونه (`postyar_connector_*`)، پاک‌سازی cron eventها و meta محصول؛ **نمی‌سازد** رد سمت پُستیار — سایت در داشبارد بماند تا تاریخچه حفظ شود؛ حذف دستی: `DELETE /wordpress/sites/:id` (Revoke).
