...
پُست‌یار v1.0.0 — اولین انتشار کامل پلتفرم. این یادداشت فقط قابلیت‌های واقعاً تحویل‌شده و محدودیت‌های واقعی را ثبت می‌کند (master prompt §208-210 — هیچ موفقیت ساختگی گزارش نمی‌شود).
...
5. Bale هم secret_token وب‌هوک را verify نمی‌کند ⇒ پُست‌یار روی مسیر bale هدر `X-Postyar-Secret` را اجباری می‌کند (سند در WORDPRESS/README پرووایدرها).
