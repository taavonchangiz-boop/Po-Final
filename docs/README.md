...
Independent SaaS rebuild from frozen read-only reference audits (`docs/audits/`). Version **1.2.0**.
...
| `postelrobbal/wordpress-plugin/postyar-connector/` | Official WP/WooCommerce connector plugin (see [WORDPRESS.md](WORDPRESS.md) for current status) |
...
| `docs/*.md` | All engineering/deployment reports — kept OUTSIDE the two host-facing folders so uploads stay clean (items 1–2 of feedback round 15) |
| `docs/audits/` | Forensic audits of the reference systems (read-only research corpus) |
| `docs/assets/brand/` | Vazirmatn fonts, PWA icons, logos (canonical brand kit — the app ships only this official logo) |
